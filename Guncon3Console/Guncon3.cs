    ...
    internal static class Guncon3
    {
        ...
        internal static void Connect()
        {
            try
            {
                ...
                var devInfo = USBDevice.GetDevices(deviceguid).FirstOrDefault(x => x.PID == pid && x.VID == vid);
                if (devInfo == null)
        ...
        private static List<byte> guncondecode(byte[] data2)
        {
            ...
            if (data2.Length != 15) return ret;
            for (int i = 0; i < 15; i++) data[i] = data2[i];
            ...
            for (x = 12; x >= 0; x--)
            {
                ...
                for (y = 4; y > 1; y--)
                {
                    ...
                    if ((bkey & 3) == 0) _byte = (_byte - bkey) - keyr;
                    else if ((bkey & 3) == 1) _byte = (_byte + bkey) + keyr;
                    else _byte = (_byte ^ bkey) ^ keyr;
                }
                ret.Add((byte)_byte);
            }
            return ret;
